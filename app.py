    ...
    def stderr_reader() -> None:
        ...
        for line in process.stderr:
            ...
            if duration and current is not None and duration > 0:
                ...
                message = f"处理中 {percent}% · 已用时 {elapsed}s"
            else:
                percent = min(95, 5 + elapsed % 90)
                snippet = line.strip()[:120] or "正在处理..."
                message = f"{snippet} · 已用时 {elapsed}s"
...
def _run_worker(
    ...
    try:
        ...
        _set_run_status(
            running=False,
            percent=100 if result["success"] else result.get("percent", 0),
            message="处理完成。" if result["success"] else result.get("error", "处理失败。"),
...
def run_feature():
    ...
    try:
        ...
        threading.Thread(
            target=_run_worker,
            args=(feature, values, input_path, output_path, binaries, custom_args),
            daemon=True,
        ).start()
        return jsonify({"ok": True, "message": "处理任务已启动。"})
    except Exception as exc:
        return jsonify({"ok": False, "message": str(exc)}), 400
