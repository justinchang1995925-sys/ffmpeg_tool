...
FEATURES = [
    ...
    },
    {
        ...
        "category": "音频处理",
        "title": "PCM 裸流转 WAV",
        "description": "PCM 是无文件头的裸音频数据，需手动指定采样格式、采样率和声道数才能转换。默认参数适配语音识别管线（16k/单声道/s16le）。",
        "tool": "ffmpeg",
        "output_ext": "wav",
        "example": "ffmpeg -y -f s16le -ar 16000 -ac 1 -i input.pcm output.wav",
        "docs": [
            "-f：输入采样格式，PCM 无文件头必须显式指定，s16le 表示 16-bit 小端。",
            "-ar：采样率，例如 16000，必须与录制时一致，否则会变调。",
            "-ac：声道数，1=mono，2=stereo，填错会导致播放忽快忽慢。",
            "这三个参数必须放在 -i 之前，用于告诉 FFmpeg 如何解析裸流。",
        ],
        "pre_input_args": ["-f", "{sample_format}", "-ar", "{sample_rate}", "-ac", "{audio_channels}"],
        "args": [],
        "params": [
            {
                "name": "sample_format",
                "label": "采样格式 -f",
                "default": "s16le",
                "choices": ["s16le", "s16be", "u8", "s24le", "s32le", "f32le"],
                "help": "最常见为 s16le（16-bit 小端）。",
            },
            {
                "name": "sample_rate",
                "label": "采样率 -ar",
                "default": "16000",
                "placeholder": "16000",
                "help": "单位 Hz，须与录制时一致。常见值：16000、44100、48000、8000。",
            },
            {
                "name": "audio_channels",
                "label": "声道数 -ac",
